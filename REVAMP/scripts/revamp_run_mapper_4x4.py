...
def main():
    ...
    os.system('cp -r '+REVAMP_APPLICATIONS+'/applications_4x4 '+ REVAMP_MAPPER_HOME+'/applications')

    applications = os.listdir(REVAMP_APPLICATIONS+'/applications_4x4')
    ...
    for app in applications:
        ...
        print('src/build/homogeneous_compiler -d applications/'+app+ ' -j '+REVAMP_HOMOGENEOUS_ARCHITECTURE + '/hycube_original_4x4_torus.json -x ' +str(x)+ ' -y '+str(y)+' -i 4')
        outlog.write('src/build/homogeneous_compiler -d applications/'+app+ ' -j '+REVAMP_HOMOGENEOUS_ARCHITECTURE + '/hycube_original_4x4_torus.json -x ' +str(x)+ ' -y '+str(y)+' -i 4\n')
        os.system('src/build/homogeneous_compiler -d applications/'+app+ ' -j '+REVAMP_HOMOGENEOUS_ARCHITECTURE + '/hycube_original_4x4_torus.json -x ' +str(x)+ ' -y '+str(y)+' -i 4  |grep "MAPPING_RESULT"| cut -d "=" -f 2|grep -o \'[0-9]*\'>homo_II')
        with open('homo_II', 'r') as file:
            ii = file.read().replace('\n', '')
        
        print(app+'  -->  '+ii)
        outlog.write("Homogeneous baseline-> "+app+'  -->  II='+ii+'\n')
        #filex.write('hycube_original_4x4_torus.json'+','+'app+'  II='+ii)
        if int(ii)>int(maxII):
            maxII=ii
        totalhm+=float(freq*node_count[app]/int(ii))
    filex.write('hycube_original_4x4_torus'+','+str(totalhm/len(applications))+'\n')
    #homogeneous['hycube_original_4x4_torus.json']=totalhm/len(applications)
