source -echo -verbose ./dc_setup_4x4.tcl
...
set compile_state_reachability_high_effort_merge    true
...
set mux_auto_inferring_effort           4
set register_duplicate                  true
#
set variloglib_sdf3.0_support           true
set varilogout_higher_designs_first     true
...
#analyze -format sverilog ${RTL_SOURCE_FILES} -vcs "+define+NUM_HOPS_PER_CYCLE=4+ARCH88888888+HETERO+HETERO_COMPUTE+HETERO_NETWORK+HETERO_CONFIG"
#analyze -format sverilog ${RTL_SOURCE_FILES} -vcs "+define+NUM_HOPS_PER_CYCLE=4+ARCH88888888"
#analyze -format sverilog ${RTL_SOURCE_FILES} -vcs "+define+NUM_HOPS_PER_CYCLE=4+ARCH88888888+HETERO+HETERO_COMPUTE+HETERO_NETWORK"
analyze -format sverilog ${RTL_SOURCE_FILES} -vcs "+define+NUM_HOPS_PER_CYCLE=4+ARCH88888888+${DEFINES}"
...
#write -hierarchy -format verilog -output ${RESULTS_DIR}/hycube.test.elab.v
#################################################################################
...
#################################################################################
    ...
    #read_saif -auto_map_names -input hycube.saif -instance hycube -verbose
    #read_saif -auto_map_names -input ${DCRM_SAIF_INPUT_FILE} -instance network_fixed_flexible_tb/chip/hycube -verbose
...
#write hierarchy -format verilog -output ${RESULTS_DIR}/hycube.test_elab.v 
...
#
#set variloglib_sdf3.0_support           true
et variloglib_sdf3.0_support           true
set varilogout_higher_designs_first     true
set verilogout_show_unconnected_pins    true
#current_design ${DESIGN_TOPLEVEL}
define_name_rules nameRules -restricted "!@#$%^&*()\\-" -case_insensitive
...
#write -hierarchy -format verilog -output ${RESULTS_DIR}/hycube.test_elab1.v
...
#write -hierarchy -format verilog -output ${RESULTS_DIR}/hycube.test_elab2.v
##################################################################################
...
#report_area -hierarchy > ${REPORTS_DIR}/hycube_area_hier.rpt
#report_resources -hierarchy > ${REPORTS_DIR}/${DCRM_FINAL_RESOURCES_REPORT}
#report_area   > ${REPORTS_DIR}/hycube_area.rpt
## report_clock_gating -nosplit > ${REPORTS_DIR}/${DCRM_FINAL_CLOCK_GATING_REPORT}
#}
#
...
#
#
report_power -nosplit -groups {memory black_box register sequential combinational} > ${REPORTS_DIR}/${DCRM_FINAL_POWER_REPORT}
#report_power -hierarchy -analysis_effort medium > ${REPORTS_DIR}/hycube.power.hier.rpt
#report_power -cell -flat > ${REPORTS_DIR}/hycube.power.cell.rpt
#report_power -net > ${REPORTS_DIR}/hycube.power.net.rpt
## report_clock_gating -nosplit > ${REPORTS_DIR}/${DCRM_FINAL_CLOCK_GATING_REPORT}
#
